...
- 30분: 새 공고 빠르게 반영
- 위비티 full: **1페이지만** 크롤 후 `crawl_state`에 다음 페이지 저장. 매일 1회 → ~100일이면 전체 수집
- **crawl_state 테이블 필요**: `supabase/migrations/20260222_crawl_state.sql` 적용 또는 SQL Editor에서 직접 생성
...
| Crawl 위비티 (Full Daily) | 매일 3시 UTC | 위비티 1페이지만 크롤 (crawl_state로 순차, ~100일이면 전체 수집) |
